...
export async function completeChat(request: Omit<ChatRequest, 'model'> & { model?: string }): Promise<string> {
  const model = request.model || appConfig.model
  if (!appConfig.baseUrl || !model || !appConfig.apiKey) throw new Error('LLM Gateway 配置不完整，请检查 .env.local。')

  const controller = new AbortController()
  const timeout = setTimeout(() => controller.abort(), appConfig.requestTimeoutMs)
  try {
    const response = await fetch(`${appConfig.baseUrl}/v1/chat/completions`, {
      method: 'POST',
    ...
    if (error instanceof Error && error.name === 'AbortError') throw new Error(`模型请求超过 ${Math.round(appConfig.requestTimeoutMs / 1000)} 秒，已自动停止。`)
    throw error
  } finally {
