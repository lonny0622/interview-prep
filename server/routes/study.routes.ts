...
export async function handleStudyRoutes(request: IncomingMessage, response: ServerResponse): Promise<boolean> {
  if (matchesRoute(request, 'POST', '/api/learning-sessions')) {
    try {
    ...
    } catch (error) { jsonResponse(response, 400, { error: errorMessage(error, '学习 session 创建失败。') }); return true }
  }
  ...
  if (matchesRoute(request, 'POST', '/api/practice-sessions')) {
    ...
    } catch (error) { jsonResponse(response, 400, { error: errorMessage(error, '刷题 session 创建失败。') }); return true }
  }
  if (matchesRoute(request, 'POST', '/api/practice-answers')) {
    try {
    ...
    } catch (error) { jsonResponse(response, 400, { error: errorMessage(error, '回答保存失败。') }); return true }
  }
  return false
}
