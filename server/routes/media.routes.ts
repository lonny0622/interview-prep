...
export async function handleMediaRoutes(request: IncomingMessage, response: ServerResponse, config: SpeechConfig, services: MediaServices): Promise<boolean> {
  if (matchesRoute(request, 'GET', '/api/speech/health')) { jsonResponse(response, 200, { configured: Boolean(config.sttBaseUrl && config.sttModel && config.sttApiKey), provider: config.sttProvider || 'openai-compatible', model: config.sttModel || '' }); return true }
  if (matchesRoute(request, 'POST', '/api/resume/extract')) {
    try {
  ...
  if (matchesRoute(request, 'POST', '/api/stt/transcribe')) {
    try {
