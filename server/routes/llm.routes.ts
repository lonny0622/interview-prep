...
export async function handleLlmRoutes(request: IncomingMessage, response: ServerResponse, config: LlmConfig, services: LlmServices): Promise<boolean> {
  if (is(request, 'GET', '/api/llm/health')) {
    ...
    } catch (error) { jsonResponse(response, 502, { ok: false, configured: true, provider: config.provider, model: config.model, error: error.message }); return true }
  }
  if (is(request, 'POST', '/api/llm/parse-questions')) {
    ...
    } catch (error) { jsonResponse(response, 502, { error: error.message || 'LLM 解析失败。' }); return true }
  }
  if (is(request, 'POST', '/api/llm/enrich-questions')) {
    ...
    } catch (error) { jsonResponse(response, 502, { error: error.message || '题目内容生成失败。' }); return true }
  }
  if (is(request, 'POST', '/api/score-answer')) {
    try {
      ...
      let score
      try { score = { ...(await services.scoreAnswer(body.question, body.answer)), source: 'llm' } } catch (error) { score = { ...services.fallbackScore(body.question, body.answer), fallbackReason: error.message } }
      jsonResponse(response, 200, { score, model: config.model }); return true
    } catch (error) { jsonResponse(response, 400, { error: error.message || '评分失败。' }); return true }
  }
  return false
}
