...
export async function handleQuestionRoutes(request: IncomingMessage, response: ServerResponse): Promise<boolean> {
  ...
  if (is(request, 'PATCH', /^\/api\/categories\/[^/]+$/)) {
    try { const category = updateCategory(idFrom(request), (await readJson<{ name?: string }>(request)).name); jsonResponse(response, category ? 200 : 404, category ? { category } : { error: '分类不存在。' }); return true } catch (error) { jsonResponse(response, errorCode(error) === 'CATEGORY_EXISTS' ? 409 : 400, { error: errorMessage(error, '分类更新失败。') }); return true }
  }
  if (is(request, 'DELETE', /^\/api\/categories\/[^/]+$/)) {
    try { const deleted = deleteCategory(idFrom(request)); jsonResponse(response, deleted ? 204 : 404, deleted ? {} : { error: '分类不存在。' }); return true } catch (error) { jsonResponse(response, errorCode(error) === 'CATEGORY_IN_USE' ? 409 : 400, { error: errorMessage(error, '分类删除失败。') }); return true }
  }
  ...
  if (is(request, 'PATCH', /^\/api\/questions\/[^/]+$/)) {
    try { const updated = editQuestion(idFrom(request), await readJson(request)); jsonResponse(response, updated ? 200 : 404, updated ? { question: updated } : { error: '题目不存在。' }); return true } catch (error) { jsonResponse(response, 400, { error: errorMessage(error, '题目更新失败。') }); return true }
  }
