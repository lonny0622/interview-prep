...
export async function handleProfileRoutes(request: IncomingMessage, response: ServerResponse, services: ProfileServices, config: ProfileRouteConfig): Promise<boolean> {
  ...
  if (matchesRoute(request, 'PATCH', /^\/api\/profile\/jobs\/[^/]+$/)) {
    try { const job = updateJobProfile(lastPathSegment(request), await readJson(request)); jsonResponse(response, job ? 200 : 404, job ? { job } : { error: '岗位不存在。' }); return true } catch (error) { jsonResponse(response, 400, { error: errorMessage(error, '岗位更新失败。') }); return true }
  }
  ...
  if (matchesRoute(request, 'POST', /^\/api\/profile\/jobs\/[^/]+\/resumes$/)) {
    try {
      ...
      const resume = createResume(pathSegment(request, 4), body)
  ...
  if (matchesRoute(request, 'PATCH', /^\/api\/profile\/resumes\/[^/]+$/)) {
    try { const resume = updateResume(lastPathSegment(request), await readJson(request)); jsonResponse(response, resume ? 200 : 404, resume ? { resume } : { error: '简历不存在。' }); return true } catch (error) { jsonResponse(response, 400, { error: errorMessage(error, '简历更新失败。') }); return true }
  }
