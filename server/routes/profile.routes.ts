...
export async function handleProfileRoutes(request: IncomingMessage, response: ServerResponse): Promise<boolean> {
  ...
  if (is(request, 'PATCH', '/api/profile')) {
    try { jsonResponse(response, 200, { profile: updateProfile(await readJson(request, 500_000)) }); return true } catch (error) { jsonResponse(response, 400, { error: error.message || '个人资料保存失败。' }); return true }
  }
  ...
  if (is(request, 'POST', '/api/profile/jobs')) {
    ...
    } catch (error) { jsonResponse(response, 400, { error: error.message || '岗位创建失败。' }); return true }
  }
  if (is(request, 'PATCH', /^\/api\/profile\/jobs\/[^/]+$/)) {
    try { const job = updateJobProfile(idFrom(request), await readJson(request)); jsonResponse(response, job ? 200 : 404, job ? { job } : { error: '岗位不存在。' }); return true } catch (error) { jsonResponse(response, 400, { error: error.message || '岗位更新失败。' }); return true }
  }
  ...
  if (is(request, 'POST', /^\/api\/profile\/jobs\/[^/]+\/resumes$/)) {
    ...
    } catch (error) { jsonResponse(response, 400, { error: error.message || '简历保存失败。' }); return true }
  }
  if (is(request, 'PATCH', /^\/api\/profile\/resumes\/[^/]+$/)) {
    try { const resume = updateResume(idFrom(request), await readJson(request)); jsonResponse(response, resume ? 200 : 404, resume ? { resume } : { error: '简历不存在。' }); return true } catch (error) { jsonResponse(response, 400, { error: error.message || '简历更新失败。' }); return true }
  }
