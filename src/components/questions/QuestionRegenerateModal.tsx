...
export function QuestionRegenerateModal({ state, onChange, onClose, onContinue, onConfirm }: Props) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !state.processing && !state.saving) onClose() }}>
    <section className="editor-modal import-modal" role="dialog" aria-modal="true" aria-labelledby="regenerate-title">
      ...
      <div className="import-summary">{state.processing ? <Sparkles size={16} /> : <Check size={16} />} {state.processing ? (state.progress.status || `正在分批生成 ${completed}/${total} 道题目`) : `已生成 ${state.drafts.length}/${total} 道题目，确认后才会覆盖原内容`}</div>
      ...
      <div className="modal-actions">
        ...
        <button className="primary-button" type="button" disabled={state.processing || state.saving || state.drafts.length !== total} onClick={onConfirm}>{state.saving ? '正在保存…' : `确认覆盖 ${state.drafts.length} 道题`} <Check size={13} /></button>
      </div>
    </section>
  </div>
}
