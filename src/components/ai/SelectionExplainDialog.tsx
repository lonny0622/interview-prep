...
function ExplainHistory({ sessions, activeId, onSelect, onDelete }: {
  ...
  return <aside className="ai-history-panel" aria-label="AI 对话历史">
    ...
    <div className="ai-history-list">
      {!sessions.length && <div className="ai-history-empty"><MessageSquareText size={20} /><strong>还没有历史对话</strong><p>在题库或学习页选中文字并点击“问 AI”后，会话会保存在这里。</p></div>}
      ...
      <div className="ai-explain-body">
        ...
        {state ? <div className="ai-explain-conversation">
          ...
          <form className="ai-explain-input" onSubmit={(event) => { event.preventDefault(); onAsk() }}>
            <div className="ai-explain-textarea-wrap"><textarea autoFocus value={state.input} onChange={(event) => onInputChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); onAsk() } }} disabled={state.streaming} placeholder="继续追问概念、原理或实际应用…" rows={2} /><small>Enter 发送，Shift + Enter 换行</small></div>
